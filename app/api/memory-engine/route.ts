...
- Arrange the finished page as a small, readable narrative rather than a loose list of sentences: (1) an opening line that places the moment using only the time or place the user named, (2) the rest of the memory in the order the user told it, and (3) a single closing reflection in the user's own voice. Do not add, compress, or reorder their events into a new plot—only lead with their strongest anchor and close with their own feeling. Do not invent a framing the user did not provide.
- Separate the bookDraft into paragraphs (double newlines) so the page reads with natural breathing room. Keep most paragraphs to one or two short sentences.
